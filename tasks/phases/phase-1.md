...
- [x] Step 1.6: Refactor model naming and calculator boundaries if needed while keeping tests green
  ...
  - Completed work:
    - Kept provider model public names unchanged because the current surface is provider-agnostic and Phase 2-friendly.
    - Extracted weekly/session pacing ignore-window configuration into private calculator internals.
    - Added explicit tests for exact ratio boundaries `0.50`, `0.85`, `1.15`, `1.50`, and `2.00`.
  - Validation: `swift test` passes 11 XCTest cases with no warnings emitted.
