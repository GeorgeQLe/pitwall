...
- [x] Step 2.8: Refactor provider foundation boundaries if needed while keeping tests green
  ...
  - Completed result:
    - Audited the Phase 2 provider boundaries and kept this as a no-code refactor step because shared `ProviderModels.swift` remains provider-agnostic while Claude, Codex, Gemini, confidence, and secret-store behavior stay isolated in adapter-specific files.
    - Preserved clean-room constraints: no copied upstream source/assets/tests, no real provider filesystem reads, no credential extraction, no raw prompt/token persistence, and no UI behavior added in this phase.
    - Validation on 2026-04-21: `swift test` passed 29 XCTest cases with 0 failures and no warnings emitted.
  - Implementation plan for next run:
