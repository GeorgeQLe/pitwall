...
- [x] Step 4.8: Refactor v1 hardening boundaries if needed while keeping tests green
  ...
  - Completed notes:
    - Reviewed Phase 4 hardening boundaries and kept the existing architecture intact: pure retention/redaction/GitHub request logic remains in `PitwallCore`, persistence and coordination remain in `PitwallAppSupport`, and SwiftUI/AppKit presentation remains in `PitwallApp`.
    - Removed the obsolete `Sources/PitwallCore/RedPhaseStub.swift` Step 1.1 placeholder now that the core target has real implementation files.
    - Review-only lane completed locally due subagent policy; no privacy, secret-handling, or clean-room boundary regression was identified.
