...
### High
- [x] [Sources/PitwallAppSupport/CodexAuthController.swift:746-770] — `ProcessCodexCommandRunner.run(arguments:stdin:)` can resume its checked continuation with a thrown stdin close/write error after the child process has already launched, then resume again from `terminationHandler`; restructure the launch/stdin path so the continuation has a single completion path and add a regression test for stdin-write failure after launch.
