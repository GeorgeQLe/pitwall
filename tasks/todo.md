...
- [x] Phase 5 Step 5.8 boundary refactor audit recorded as "no refactor required" (macOS `swift test` holds the 193-test Step 5.7 baseline; Windows/Linux CI gap carried forward unchanged).
- [x] Phase 5 Cross-Platform V1 Parity milestone complete — ready for archival to `tasks/phases/phase-5.md` in the next session.
...
- [x] Step 5.8: Refactor cross-platform boundaries if needed while keeping tests green *(Docs-only outcome: audit cleared with "no refactor required". The Windows/Linux apparent duplication — `*StatusFormatter`, `*TrayMenuViewModel`/`*TrayMenuBuilder`, `Stored*` Codable types, `*ClaudeCredential{Input,Error}`, `*NotificationScheduler`, `*DiagnosticsExporter` — is an intentional platform-isolation boundary that Step 5.6 parity tests bind to via hard-coded `Expected` constants in both suites; collapsing would erase the cross-shell drift signal. `PitwallAppSupport` pure-Foundation files remain correctly placed (macOS-shell-only, `~/Library/Application Support/` + `UserDefaults`-bound). Recorded in `docs/cross-platform-architecture.md` "Cross-Platform Boundary Refactor Audit (Step 5.8)". macOS `swift build` + `swift test`: 193/193 pass, 0 regressions, byte-identical to Step 5.7 baseline. Windows/Linux CI gap carried forward unchanged.)*
...
- Ready for next phase: yes. Phase 5 Cross-Platform V1 Parity is complete; next session archives Phase 5 to `tasks/phases/phase-5.md` (no new phase is defined since Phase 5 was the final roadmap phase).
