...
- Step 6b.6: Fill in real Sparkle keys and appcast URL from manual prerequisites
  ...
  - Files: modify `Sources/PitwallApp/Info.plist` (replace `{{SUFeedURL}}` and `{{SUPublicEDKey}}` placeholders with real values in `.release-config.example`), update `.release-config.example` with real feed URL pattern, update `Formula/pitwall.rb` with the real tap path if using self-hosted tap.
...
- Step 6b.7: Write regression tests for Sparkle integration and release pipeline
  ...
  - Shell validation: `bash scripts/release.sh --dry-run` (add `--dry-run` flag to `scripts/release.sh` that runs through validation + build + codesign but skips notarization, GitHub release, and appcast publish — useful for CI and pre-release checks).
  - Verify `make install` still works for ad-hoc local dev with no Sparkle keys configured.

- Step 6b.8: End-to-end release validation
