...
public actor WindowsProviderConfigurationStore: ProviderConfigurationStorage {
    ...
    public func load() async -> ProviderConfigurationSnapshot {
        guard
            ...
            let stored = try? decoder.decode(StoredWindowsProviderConfiguration.self, from: data)
        else {
    ...
    public func save(_ snapshot: ProviderConfigurationSnapshot) async throws {
        ...
        let stored = StoredWindowsProviderConfiguration(snapshot: snapshot)
...
struct StoredWindowsProviderConfiguration: Codable {
    var providerProfiles: [StoredWindowsProviderProfile]
    var claudeAccounts: [StoredWindowsClaudeAccount]
    var selectedClaudeAccountId: String?
    var userPreferences: StoredWindowsUserPreferences

    init(snapshot: ProviderConfigurationSnapshot) {
        providerProfiles = snapshot.providerProfiles.map(StoredWindowsProviderProfile.init)
        claudeAccounts = snapshot.claudeAccounts.map(StoredWindowsClaudeAccount.init)
        selectedClaudeAccountId = snapshot.selectedClaudeAccountId
        userPreferences = StoredWindowsUserPreferences(snapshot.userPreferences)
    }
...
struct StoredWindowsProviderProfile: Codable {
...
struct StoredWindowsClaudeAccount: Codable {
    var id: String
    var label: String
...
struct StoredWindowsUserPreferences: Codable {
    ...
    init(_ preferences: UserPreferences) {
        ...
        notificationPreferences = StoredWindowsNotificationPreferences(preferences.notificationPreferences)
    }
...
struct StoredWindowsNotificationPreferences: Codable {
