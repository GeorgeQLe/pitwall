...
final class WindowsDiagnosticsExporterTests: XCTestCase {
    ...
    func test_export_writesJSONFileWithRedactedExport() throws {
        ...
        let exporter = WindowsDiagnosticsExporter(root: root, now: { now })
    ...
    func test_build_appliesRedactorToProviderSummaries() {
        let exporter = WindowsDiagnosticsExporter(root: root)
