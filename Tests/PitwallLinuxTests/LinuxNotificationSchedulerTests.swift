...
private final class SpyBackend: LinuxNotificationDelivering, @unchecked Sendable {
...
final class LinuxNotificationSchedulerTests: XCTestCase {
    func test_schedule_forwardsRequestToBackend() {
        let backend = SpyBackend()
        let scheduler = LinuxNotificationScheduler(backend: backend)
    ...
    func test_notificationPolicy_delegatesThroughLinuxScheduler() {
        let backend = SpyBackend()
        let scheduler = LinuxNotificationScheduler(backend: backend)
    ...
    func test_suppressedBackend_silentlyAcceptsDelivery() {
        let scheduler = LinuxNotificationScheduler(backend: LinuxNotificationSuppressedBackend())
        scheduler.schedule(
            NotificationRequest(
                kind: .expiredAuth,
                providerId: .claude,
