...
/// Phase 5 Step 5.6 Linux-side regression anchor. Every expected string,
/// redacted key set, retention outcome, and heatmap mapping mirrors the
/// sibling assertions in `WindowsCrossPlatformRegressionTests`. If either
...
final class LinuxCrossPlatformRegressionTests: XCTestCase {
    ...
    func test_providerVisibility_roundTripAndDisabledProviderIsHidden() async throws {
        ...
        let vm = LinuxTrayMenuBuilder().build(
    ...
    func test_statusFormatter_matchesSharedExpectedStrings() {
        let formatter = LinuxStatusFormatter()
    ...
    func test_secureStorageDegradedStateEnum_isVisibleToShell() {
        let degraded: LinuxSecretServiceError = .backendUnavailable
    ...
    func test_codexDetector_sanitizesAbsolutePathsAndTokenShapedSubstrings() {
        let probe = LinuxCodexRegressionProbe()
        probe.files["auth.json"] = LinuxCodexArtifactMetadata(
            relativePath: "/home/alice/.config/codex/auth.json sk-abc123DEADBEEF ghp_leak ya29.secret AIzaSyDummy",
            byteSize: -17,
        ...
        let detector = LinuxCodexDetector(root: root, probe: probe)
    ...
    func test_geminiDetector_sanitizesAbsolutePathsAndTokenShapedSubstrings() {
        let probe = LinuxGeminiRegressionProbe()
        probe.files["oauth_creds.json"] = LinuxGeminiArtifactMetadata(
            relativePath: "/home/alice/.config/gemini/oauth_creds.json ya29.DEADBEEF AIzaSyLeaked",
            byteSize: 128,
            modifiedAtEpochSeconds: 1_700_000_200
        )
        let detector = LinuxGeminiDetector(root: root, probe: probe)
    ...
    func test_detectors_suppressedWhenProbeUnavailable_noFabricatedEvidence() {
        let codexEvidence = LinuxCodexDetector(root: root, probe: LinuxCodexSuppressedProbe()).detect()
        XCTAssertTrue(codexEvidence.suppressed)
        XCTAssertTrue(codexEvidence.artifacts.isEmpty)

        let geminiEvidence = LinuxGeminiDetector(root: root, probe: LinuxGeminiSuppressedProbe()).detect()
    ...
    func test_diagnosticsExport_redactedKeySet_matchesSharedDiagnosticsContract() {
        let exporter = LinuxDiagnosticsExporter(root: root, now: { Self.fixedNow })
...
private final class LinuxCodexRegressionProbe: LinuxCodexFilesystemProbing, @unchecked Sendable {
    var files: [String: LinuxCodexArtifactMetadata] = [:]
    var directoryCounts: [String: Int] = [:]
    var isAvailable: Bool { true }
    func probe(relativePath: String) -> LinuxCodexArtifactMetadata? { files[relativePath] }
    func childCount(relativeDirectory: String) -> Int { directoryCounts[relativeDirectory] ?? 0 }
}

private final class LinuxGeminiRegressionProbe: LinuxGeminiFilesystemProbing, @unchecked Sendable {
    var files: [String: LinuxGeminiArtifactMetadata] = [:]
    var directoryCounts: [String: Int] = [:]
    var isAvailable: Bool { true }
    func probe(relativePath: String) -> LinuxGeminiArtifactMetadata? { files[relativePath] }
    func childCount(relativeDirectory: String) -> Int { directoryCounts[relativeDirectory] ?? 0 }
}
